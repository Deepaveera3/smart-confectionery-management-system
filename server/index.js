...
app.use(cors({
  origin: (origin, callback) => {
    // Permit local dev, Vercel deployments, and configured CLIENT_URL
    if (!origin || origin.includes('localhost') || origin.includes('127.0.0.1') || /\.vercel\.app$/.test(origin) || origin === process.env.CLIENT_URL) {
      callback(null, true);
    } else {
      callback(null, true);
    }
